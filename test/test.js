...
test("Testing that removeItemByID works", t => {
    ...
    t.deepEqual(
        actual,
        expected,
        "removeItemByID should remove the item with the corresponding ID from the array."
    );
    t.end();
});
...
test("Testing whether editItemOnList() edits a to-do.", t => {
    ...
    t.deepEqual(
        actual,
        expected,
        "editItemOnList() should change the title and status of the old to-do fed into it."
    );
    t.end();
});
